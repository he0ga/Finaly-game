...
public class CharactersSystem : MonoBehaviour
{
    ...
    void ChooseNewPath()
    {
        ...
        Debug.Log($"������ ����� ����: {(chooseLeftPath ? "�����" : "������")}, ��������� �����: {currentPointIndex}");
    }
    ...
    IEnumerator MovementRoutine()
    {
        while (true)
        {
            if (hasReachedLastPoint)
            {
                ...
                // Если противник дошёл до последней точки (внутри офиса) — Game Over
                ...
                // Иначе дверь была закрыта — противник отступает, выбираем новый путь
                ChooseNewPath();
                continue;
            }

            // Ждём перед переходом к следующей точке
            ...
            if (hasReachedLastPoint)
            {
                // Флаг выставлен внутри GetNextPointIndex (дверь закрыта) — обработаем в начале цикла
                continue;
            }
            ...
            // Проверяем, достигнута ли последняя точка после перемещения
            if (currentPointIndex == currentPathPoints.Length - 1)
            {
    ...
    /// <summary>Activates the Game Over panel and hides gameplay UI.</summary>
